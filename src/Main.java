...
public class Main{
    ...
    public static void main(String[] args) {
        ...
        int sizeOfRandomItemSet = rand.nextInt(AMOUNT);
        for (int i = 0; i < sizeOfRandomItemSet; i++) {
    ...
    private static String findBestNeighbour(String currentSolution, int maxWeight) {
        ...
        for (int i = 0; i < AMOUNT; i++) {
            ...
            if (currentBestWeight > maxWeight && neighbourWeight < currentBestWeight) {
                currentBestValue = neighbourValue;
                currentBestWeight = neighbourWeight;
                bestNeighbour = neighbour.toString();
            } else if (neighbourWeight == currentBestWeight && neighbourValue > currentBestValue) {
                currentBestValue = neighbourValue;
                currentBestWeight = neighbourWeight;
                bestNeighbour = neighbour.toString();
            } else if (neighbourValue > currentBestValue && neighbourWeight <= maxWeight) {
                currentBestValue = neighbourValue;
                currentBestWeight = neighbourWeight;
                bestNeighbour = neighbour.toString();
            }
